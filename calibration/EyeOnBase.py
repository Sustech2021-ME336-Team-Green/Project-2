...
def image_callback(color_image: object, depth_image: object, intrinsics: object) -> object:
        ...
        for i in range(4):
            for j in range(4):
                for k in range(4):
                    ...
                    cv2.imwrite("./images/image_"+str(i)+str(j)+str(k)+'.png',color_image)
                    depth_image = frame.depth_image[0]
                    observed_pt = image_callback(color_image, depth_image, sensor_intrinsics)
                    print("The intrinsic paras are: ", sensor_intrinsics)
                    exit()
